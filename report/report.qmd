...
We divide gross loss values by the original loan balance (origination UPB) and convert them to Bps. This provides us a value representing the percentage of a loan's original value which we might expect to be lost given a delinquency in a stressed economic scenario. Not all loans in this population experienced a loss. Using the [user guide provided by Freddie Mac](https://www.freddiemac.com/fmac-resources/research/pdf/user_guide.pdf), we restrict to loans which satisfy these conditions:
...
tibble::tibble(
  ...
  ggplot2::labs(
    ...
    caption = figure_no("Annualized Loss Values")
  ) +
  theme_plot()
```
...
ggplot2::ggplot(
  ...
  ggplot2::labs(
    ...
    caption = figure_no("Varable distribution")
  ) +
  theme_plot() +
  ggplot2::theme(
...
Before estimation, we wanted to discuss data treatments. While specific transformations (such as splines or log transforms) were applied to the different learning methods, this section focuses on considerations made for data quality issues such as missing or extreme values. Because this dataset is a cleaned research dataset made publicly available by Freddie Mac, little to no data cleaning was necessary to prepare this data for modeling. Lets discuss both numeric and categorical predictors in turn.

Below is a table indicating simple summary statistics for our three numeric predictors. These are sometimes expanded into splines, but they remain column vectors for this table. All three variables have sensible ranges and distributions while also containing no missing values. Note that Freddie Mac does include special values which indicate missing values (e.g., -9999 for FICO). Those values were handled when the data was pulled prior to any estimation or summary statistic computation.
...
train_test$train |>
  ...
  tidyr::pivot_wider() |>
  knitr::kable(
    caption = "Numeric Preditor Ranges"
  )
```

Similarly, this table indicates the shares for each of the categorical terms. Note that a given category term superset (e.g., all of the *Region:* terms) will not add up to 100%. This is because all other observations would be in the referece category. The table below simply reveals the average of the 0s and 1s present in each of the one hot encoded terms. Like the numeric predictors, all observations fall into one of the associated categories and no missing values are present.

```{r}
recipes::bake(
  object = recipes::prep(base_rec),
  new_data = train_test$train
) |>
  dplyr::select(
    one_unit,
    one_borrower,
    dplyr::matches("occ|prop|purp|term|region")
  ) |>
  tidyr::pivot_longer(
    dplyr::everything(),
...
Note that we did not transform the independent variable for our OLS model. While it is common to apply a log transformation on loss values (given their long tail) to improve assumptions including the linearity, expected residual of zero, and normality assumptions, in this case it did not reveal an improvement in the fitted versus residuals plot.

Below is a direct comparison of the log transformed and non-transformed models. The log transformed model (left) shows that the residuals have a long right tail and do not have an expected value of zero. The non-transformed model (right) however, does reveal residuals which appear more randomly scattered with consistent tails and no discernible patterns.

```{r, fig.width=6}
dplyr::bind_rows(
  None = specs$ols_resid,
  Log = specs$ols_log_resid,
  .id = "transform"
) |>
  ggplot2::ggplot(
    ggplot2::aes(
...
All of these values have a direct relationship with overfitting. However, setting these values too small means that the model will do a poor job of understanding the variance in our response variable. Thus, we tuned these values. The default values and ranges come from `tune` in `tidymodels` as to prevent arbitrary value selection. After tuning we selected values of 6, 0.1, and 20 for `tree_depth`, `learn_rate`, and `n_trees`, respectively.
...
The table below shows the results of this exercise. It is important to remember that because the rank ordering comes from different measures which prioritize different outcomes, this ranking may not be reliable. However, given computational constraints it does help us understand how OLS and XGB might prioritize these terms.
...
dplyr::bind_rows(
  ...
  tidyr::pivot_wider() |>
  dplyr::mutate(
    "Rank Change (OLS to XGB)" = purrr::map2_chr(
      OLS, Rank,
      \(x, y) y - print(which(XGB == x))
    )
  ) |>
  dplyr::select(
    Rank,
...
Second, we performed a crude comparison of feature importance using normalized OLS coefficients versus Gain from XGBoost. A better approach would be to use Shapley values by way of the `shapr` package. Shapley values provide feature importance scores for almost any model type making it trivial to compare across model structures. However, when we attempted this we quickly ran out of computational resources for the XGBoost model for even a small subset of Shapley combinations. Thus, we did not include a Shapley comparison in this report.

Finally, we would have utilized a three-way split of our data (especially if we used more observations). This would have divided our data into train, test, and validation. While there was no data leakage between train and test in this model, we did resample train and make hyperparameter decisions about each model using the training data. It would have been better to develop and tune models with train, chose the best tuning configuration using a validation set, and compare across models with test.
