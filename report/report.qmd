...
Note that we did not transform the dependent variable for our OLS model. While it is common to apply a log transformation on loss values (given their long tail) to improve assumptions including the linearity, expected residual of zero, and normality assumptions, in this case it did not reveal an improvement in the fitted versus residuals plot. Below is a direct comparison of the log transformed and non-transformed models. The log transformed model (left) shows that the residuals have a long tail. The non-transformed model (right) however, does reveal residuals which appear more randomly scattered with consistent tails and no discernible patterns.
...
The table below shows the results of this exercise. It is important to remember that because the rank ordering comes from different measures which prioritize different outcomes, this ranking comes with limitations. However, it does help us understand how OLS and XGB prioritize these terms.
