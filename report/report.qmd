...
This research data from Freddie Mac is generally cleaned and ready for use. However, this section focuses on considerations made when determining the best model sample for our use case. We discuss which data exclusions were applied, how missing values were handled, and a robust check for extreme values. Lets begin by discussing model exclusions. Exclusions are important because they define the universe for which our model is estimated. Sample selection determines which observeraions are in our training data and greatly impact the correlation relationships we identify.

We applied the following exclusions to our data. The first two exclusions set the universe for the types of mortgage losses we are interested in modeling. These `zero_bal_code` codes restrict to losses which result from a third party sale, short sale, charge off, REO disposition, or whole loan sale. Similarly, the `net_loss` filter ensures that all loss fields are complete, and therefore we can compute a truthful gross loss value. We also selected the 2007 vintage as it represents the peak of mortgage losses. We further discuss justifications for all of these loss universe exclusions in the *Target Variable* section below.

We also handled exclusions for missingness. Freddie Mac has denoted special missing values for several predictors we are interested in using in our models. Those are denoted below as filters for `dti`, `cltv`, `fico`, and `one_borrower`. In total these exclusions remove `r exclude$pct` of our data or a total of `r exclude$n` observations.

```{sql, echo=TRUE}
#| eval: FALSE
-- Exclusions for loss universe
zero_bal_code in ('02', '03', '09', '15')
and net_loss != 0

-- Exclusisons for missingness
and dti != 999
and cltv != 999
and fico != 9999
...
Similarly, this table indicates the shares for each of the categorical terms. Note that a given category term superset (e.g., all of the *Region:* terms) will not add up to 100%. This is because all other observations would be in the reference category. The table below simply reveals the average of the 0s and 1s present in each of the one hot encoded terms. Like the numeric predictors, all observations fall into one of the associated categories and no missing values are present.
